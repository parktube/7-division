...
#### WebMCP Phase 1 Integration (Chrome Early Preview)

브라우저 내장 AI 에이전트가 CAD 뷰어를 직접 조작할 수 있는 WebMCP 통합입니다.

- **Model Context API**: `navigator.modelContext` 감지 및 도구 등록
  - Chrome 146+ WebMCP Early Preview 지원
  - `navigator.modelContext` 경로 사용 (Chrome 147 Canary 검증)

- **WebMCP 도구 4종**:
  - `viewer.get_status` - 연결 상태/버전 정보 조회
  - `viewer.get_scene_summary` - 씬 요약 (엔티티 수, 타입별 통계)
  - `viewer.get_selection` - 현재 선택된 엔티티 ID 조회
  - `viewer.select_entities` - 엔티티 선택 (replace/add 모드, 중복 제거)

- **Opt-in 토글**: localStorage 기반 활성화/비활성화
  - `useSyncExternalStore` 기반 상태 관리
  - StatusBar에 토글 버튼 UI

- **UIContext 전역 접근**: React 외부에서 UI 상태 읽기
  - `getUIStore()` / `getUIActions()` 전역 접근자
  - WebMCP 도구에서 선택 상태 읽기/쓰기

### Documentation

- [ADR-009](docs/adr/009-webmcp-integration.md): WebMCP 통합 결정
- [RFC](docs/rfc/webmcp-integration.md): WebMCP Integration 설계 문서
- [Phase 1 Plan](docs/analysis/webmcp-phase1-plan.md): 구현 계획
- [WebMCP Notes](docs/references/webmcp-early-preview-notes.md): Chrome Early Preview 참고 노트

### Tests

- `store.test.ts` - localStorage 토글 스토어 테스트 (12 tests)
- `tools.test.ts` - WebMCP 도구 단위 테스트 (15 tests)
- `types.test.ts` - 타입 유틸리티 테스트 (8 tests)
...
*작성: 2026-01-09 | 최종 업데이트: 2026-02-25*
