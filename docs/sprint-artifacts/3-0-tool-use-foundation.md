  ...
  exec(toolName: string, input: Record<string, unknown>): ToolResult {
    try {
      ...
      switch (toolName) {
        ...
        case 'set_stroke':
          return {
            success: this.scene.set_stroke(input.name as string, this.styleToJson(input.stroke)),
            entity: input.name as string
          };

        case 'set_fill':
          return {
            success: this.scene.set_fill(input.name as string, this.styleToJson(input.fill)),
            entity: input.name as string
          };
